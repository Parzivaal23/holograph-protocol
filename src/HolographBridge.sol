 ...
 * @notice Bridge any holographable contracts and assets across blockchains
...
contract HolographBridge is Admin, Initializable, HolographBridgeInterface {
  ...
  /**
   * @notice Receive a bridge from another chain
  ...
  function bridgeInRequest(
    ...
    if (hTokenValue > 0 && hTokenRecipient != address(0)) {
       ...
       *      recipient can bridge the asset to origin chain and unwrap for native gas token at any time
       */
      require(
  ...
  /**
   * @notice Create a bridge request for a destination chain
   * @dev This function works for deploying contracts and bridging supported holographable assets across chains
